...
Upgrading the runtime to Node.js 24 exposed a regression in the SQLite native addon dependency. Corpus still selected a pre-Node-24 major of `better-sqlite3`.

## Evidence

`package.json` declared `better-sqlite3` as `^11.3.0`. Native addons built or downloaded for an older Node ABI cannot be reused safely by Node 24.
