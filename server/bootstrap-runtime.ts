...
export async function bootstrapCorpus(): Promise<CorpusRuntime> {
  const scope = new CleanupScope();
  const logging = installLogging();
  scope.defer(() => logging.shutdown());
  try {
    ...
    const { startCorpus } = await import('./index.ts');
    const runtime = await startCorpus();
    scope.defer(() => runtime.close());
    return { close: () => scope.close() };
  } catch (error) {
    try { await scope.close(); }
    catch (cleanupError) { throw new AggregateError([error, cleanupError], 'Corpus bootstrap and cleanup failed'); }
    throw error;
  }
}
